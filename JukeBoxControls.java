...
public class JukeBoxControls extends JPanel{
	...
	//This will set up the gui for juke box
	...
	// Obtain and store the audio clips to play
		try
		{
			...
			url4 = new URL("file", "localhost", "newAgeRyth.wav");
		...
		String[] musicNames = {"Make A Selection...", "Western Beat", "Classical Melody", "Jeopardy Theme",
				"New Age Rythm", "Eighties Jam", "Alfred Hitchcock's Theme"};
	...
	private class ComboListener implements ActionListener{
		//Stops playing the current selection if any and resets the current selection to the one chosen
	...
	private class ButtonListener implements ActionListener{
		//Stops the current selection in either case. IF the play button was pressed start playing it again
	public void actionPerformed(ActionEvent event){
		if (current != null)
			current.stop();
